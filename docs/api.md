...
| Method | Description |
|--------|-------------|
| `get(mixed $model)` | Read value from model via getter or property |
| `set(mixed $model, mixed $value)` | Write value to model via setter or property |
...
| Any | Any | Cast to the target scalar type; scalar to `BackedEnum` via `::from()` |
